...
class YoloLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, label: torch.Tensor):  # call function
        ...
        # === 1: Slices out the coordinates we need.
        #       Layout: C=18, B=2.
        ...
        coord_loss = self.mse(  # an example print() coord_loss = 142.8409
            obj_mask * best_xywh[..., :2], obj_mask * label_xywh[..., :2]
        )
        coord_loss += self.mse(  # an example print() coord_loss = 198.6252
        ...
        #       Confidence Loss           #
        # =============================== #
        conf_obj_loss = self.mse(  # Example print conf_obj_loss = 11.6718
            obj_mask * best_pc, obj_mask * best_iou
        )

        best_pred_pcs = torch.cat([pred_box1_pc, pred_box2_pc], dim=-1)  # (64, 7, 7, 2)
        noobj_loss = self.mse(  # example print = 322.5405
        ...
        #       Classification Loss       #
        # =============================== #
        class_loss = self.mse(  # ex print = 436.0011
        ...
        loss = (  # example print loss = 19.9872
            self.lambda_coord * coord_loss  #   <- Localization Loss    | for the 'responsible' box either box1 or box2.
            + conf_obj_loss  #                  <- Confidence Loss      | where an object is present.
            + self.lambda_noobj * noobj_loss  # <- Confidence Loss      | where *no* object is present.
            + class_loss  #                     <- Classification Loss  | What is the object? Indices (0-17), compared to true label?
