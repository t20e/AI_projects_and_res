...
def extract_bboxes(t:torch.Tensor, config:Namespace):
    """
    Extract bounding boxes from a tensor (typically the models prediction), into flat (N, 9) tensor: [i, j, b, class_idx, pc, x, y, w, h]. The return tensor will have bboxes sorted by pc (descending).
    ...
    """
    S, B, DEVICE = config.S, config.B, config.DEVICE

    # --- 1: create new tensors to store class probs, first bbox and second bbox from every cell.
    # TODO fix HARDCODED NUMBER FOR 3 lines below
    class_probs = t[..., :18] # (7, 7, 18)
    bbox_1 = t[..., 18:23] # (7, 7, 5)
    bbox_2 = t[..., 23:28] # (7, 7, 5)
    ...
    # note: example variable prints visulaized @ matrices_visualize/i_j_b_coords.py
    i_coords, j_coords = torch.meshgrid(torch.arange(S, device=DEVICE), torch.arange(S, device=DEVICE), indexing='ij')  # (7, 7)
    ...
    full = torch.cat([metadata.float(), cls_coords.float(), bboxes], dim=-1)  # (7, 7, 2, 9)

    full = full.view(-1, 9)  # ( N = S*S*2, 9)    # Visualized @ matrices_visualize/bboxes_extract_full.py, but its not sorted

    # --- 5: Sort the bboxes with the highest probability at the beginning.
    sorted_indices = full[:, 4].argsort(descending=True)# Sort by pc (probability score) -> index 3
    return full.index_select(0, sorted_indices) #  Visualized @ matrices_visualize/sorted_bboxes_extracted.py
...
def reconstruct_tensor(bboxes: torch.Tensor, config):
    ...
    """
    S, B, C, DEVICE, NUM_NODES_PER_CELL = config.S, config.B, config.C, config.DEVICE, config.NUM_NODES_PER_CELL
    ...
    for box in bboxes:
        # extract the values
        i, j, b, class_idx, pc, x, y, w, h = box.long()[:3].tolist() + box[3:].tolist() # I could've done = box but that could cause indexing issues.

        # Set the class probability index -> class_idx = 1
        out[i, j, int(class_idx)] = 1.0

        # Set box info
        box_offset = C + int(b) * 5 # offset the placement of the box for whether it belongs in box1 or box2 of that cell.
        out[i, j, box_offset : box_offset + 5] = torch.tensor([pc, x, y, w, h], device=DEVICE)
